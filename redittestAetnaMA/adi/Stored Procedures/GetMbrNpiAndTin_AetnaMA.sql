...
CREATE PROCEDURE [adi].[GetMbrNpiAndTin_AetnaMA] (@LoadDate DATE, @RowStatus INT,@ClientKey INT) -- [adi].[GetMbrNpiAndTin_AetnaMA]'2022-01-01',0,3

AS
					-- DECLARE @LoadDate DATE = '2021-12-01' DECLARE @RowStatus INT = 0 DECLARE @ClientKey INT = 3
					...
					INSERT INTO #Pr(NPI,AttribTIN,ClientNPI,ClientTIN,MemberID) -- DECLARE @LoadDate DATE = '2021-12-01' DECLARE @RowStatus INT = 0 DECLARE @ClientKey INT = 3
					SELECT	pr.NPI
							,pr.AttribTIN
					...
					FROM		(SELECT		DISTINCT CONVERT(VARCHAR(50), CONVERT(NUMERIC(20, 0), adi.MEMBER_ID)) AS MEMBER_ID
								 ...
								 JOIN		ACECAREDW.adi.MbrAetMaTx adi
