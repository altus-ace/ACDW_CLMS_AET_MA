...
CTETIN AS (
SELECT AttribTIN, 
                AltNPIs = STUFF((SELECT ', '+ CONCAT(NPI,'(',LastName,')') FROM ACECAREDW.adw.tvf_AllClient_ProviderRoster (1,getdate(),1) t1
                WHERE t1.AttribTIN = t2.AttribTIN
                FOR XML PATH('')),1,1,'') FROM ACECAREDW.adw.tvf_AllClient_ProviderRoster (1,getdate(),1) t2
)
...
FROM (
                ...
                FROM [adw].[FailedMembership] f
					 LEFT JOIN ACECAREDW.adw.tvf_AllClient_ProviderRoster (1,getdate(),0) r
                ON  f.NPI       = r.NPI
                JOIN CTE l
