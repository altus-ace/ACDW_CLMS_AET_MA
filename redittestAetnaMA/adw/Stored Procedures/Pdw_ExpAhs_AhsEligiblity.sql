...
BEGIN
	/* objective 
		...
		*/
	DECLARE @FirstMemberMonth DATE = '01/01/2021';  /* this is the first month that has valid data, earlier data is not REAL */
	...
	/* PREP */					
	INSERT INTO ast.DatesToProcess_AhsExpElig (LoadDate, FirstDayOfMonth, LastDayOfMonth,Clientkey)
	SELECT mbr.LoadDate, FirstDay.dDate, LastDay.dDate, mbr.ClientKey
	FROM (SELECT mbr.RwEffectiveDate LoadDate, mbr.ClientKey
		   FROM adw.fctMembership mbr	
		   WHERE mbr.RwEffectiveDate  >= @FirstMemberMonth 
		   GROUP BY mbr.RwEffectiveDate, mbr.ClientKey
		   ) mbr
	...
	BEGIN 
	   ...
	   BEGIN -- 1 row per member per month,
		  INSERT INTO   ast.AhsExpElig_MbrCsPlanInfo (CMK,CsPlanName,CsPlanEffDate, CsPlanExpDate, FctMbrRwEffDate,FctMbr_Skey,mbrCsPlanDim_SKey, ClientKey)        
		  SELECT fm.ClientMemberKey
			 , c.MbrCsSubPlanName
			 , @StartDate, @StopDate, fm.rwEffectiveDate, fm.fctMembershipSKey, fm.MbrCsPlanKey, fm.ClientKey
	       FROM adw.FctMembership fm			 
			JOIN adw.mbrCsPlan c ON fm.MbrCsPlanKey = c.mbrCsPlanKey
	...
	/*xport to Adw */
    INSERT INTO adw.AhsExpEligiblity (CreatedDate, CreatedBy, LastUpdatedDate, LastUpdatedBy, ClientMemberKey, ClientKey, Exported 
    ...
    SELECT e.CreatedDate, e.CreatedBy, e.LastUpdatedDate, e.LastUpdatedBy, e.ClientMemberKey, e.ClientKey, 0,
