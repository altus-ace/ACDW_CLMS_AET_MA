		...
		SELECT B1.[SEQ_CLAIM_ID]
		  ...
		  INNER JOIN
			(
				...
				UNION
				--SELECT DISTINCT C4.SEQ_CLAIM_ID, C4.DIAGCODE AS VALUE_CODE, L34.VALUE_SET_NAME
				--FROM adw.Claims_Diags C4
				--INNER JOIN
				--(
				--	SELECT DISTINCT VALUE_CODE, VALUE_SET_NAME
				--	FROM [adw].[2020_tvf_Get_ClaimsByValueSet] ('Hospice','ESRD','','',@PrimSvcDate_Start,@PrimSvcDate_End,@CodesetEffDate) L3
				--) L34 ON L34.VALUE_CODE = C4.DIAGCODE
				SELECT DISTINCT C4.SEQ_CLAIM_ID, L34.VALUE_CODE, L34.VALUE_SET_NAME
				FROM adw.Claims_Diags C4
				INNER JOIN
				(
					SELECT DISTINCT VALUE_CODE, VALUE_SET_NAME, VALUE_CODE_SYSTEM, VALUE_SET_OID
