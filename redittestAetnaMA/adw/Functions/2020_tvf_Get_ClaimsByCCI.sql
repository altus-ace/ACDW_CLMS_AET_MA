





CREATE  FUNCTION [adw].[2020_tvf_Get_ClaimsByCCI]
(  
 @PrimSvcDate_Start VARCHAR(20), 
 @PrimSvcDate_End   VARCHAR(20),
 @CodesetEffDate	VARCHAR(20)
)
RETURNS TABLE
AS RETURN
(
		SELECT B1.[SEQ_CLAIM_ID]
			  ,B1.[SUBSCRIBER_ID]
			  ,B1.[CATEGORY_OF_SVC]
			  ,B1.[PRIMARY_SVC_DATE]
			  ,B1.[SVC_TO_DATE]
			  ,B1.[CLAIM_THRU_DATE]
			  ,B1.[VEND_FULL_NAME]
			  ,B1.[PROV_SPEC]
			  ,B1.[IRS_TAX_ID]
			  ,B1.[DRG_CODE]
			  ,B1.[BILL_TYPE]		
			  ,B1.[ADMISSION_DATE]
			  ,B1.[CLAIM_TYPE]
			  ,B1.[TOTAL_BILLED_AMT]
			  ,B1.[TOTAL_PAID_AMT]
			  ,D.VALUE_CODE as ValueCode
			  ,D.VALUE_CODE_SYSTEM as ValueCodeSystem
			  ,D.VALUE_SET_OID as ValueSetOID
			  ,DATEDIFF(dd,B1.PRIMARY_SVC_DATE, B1.SVC_TO_DATE)		AS LOS
			  ,DATEDIFF(dd,B1.PRIMARY_SVC_DATE, GETDATE())			AS DaysSincePrimarySvcDate
		  FROM [adw].[Claims_Headers] B1
		  INNER JOIN
			(
				SELECT DISTINCT C3.SEQ_CLAIM_ID, L33.VALUE_CODE, L33.VALUE_CODE_SYSTEM, L33.VALUE_SET_OID
				FROM adw.Claims_Diags C3
				INNER JOIN
				(
					SELECT DISTINCT VALUE_CODE, VALUE_CODE_SYSTEM, VALUE_SET_OID
					FROM lst.List_HEDIS_CODE L3
					WHERE LEFT(L3.VALUE_SET_NAME,7) IN ('ACE CCI')
					AND L3.ACTIVE = 'Y'
					AND VALUE_CODE_SYSTEM IN('ICD9CM', 'ICD10CM')
					AND @CodesetEffDate BETWEEN L3.EffectiveDate AND L3.ExpirationDate
				) L33 ON L33.VALUE_CODE = C3.DIAGCODE
			) AS D 
		ON D.SEQ_CLAIM_ID = B1.SEQ_CLAIM_ID
		WHERE B1.SEQ_CLAIM_ID IS NOT NULL
		--AND B1.PROCESSING_STATUS	= 'P'
		--AND B1.CLAIM_STATUS			= 'P'
		AND CONVERT(DATETIME, B1.PRIMARY_SVC_DATE)	>= 	@PrimSvcDate_Start
		AND CONVERT(DATETIME, B1.PRIMARY_SVC_DATE)	<=  @PrimSvcDate_End
)

/***
Usage: 
SELECT *
FROM [adw].[2020_tvf_Get_ClaimsByCCI] ('01/01/2019','12/31/2019','01/01/2019')

SELECT DISTINCT SUBSCRIBER_ID, COUNT(DISTINCT PRIMARY_SVC_DATE) as Visits, SUM(TOTAL_PAID_AMT) as PaidAmt
	FROM [adw].[2020_tvf_Get_ClaimsByCCI] ('01/01/2019','12/31/2019','01/01/2019')
GROUP BY SUBSCRIBER_ID
***/

