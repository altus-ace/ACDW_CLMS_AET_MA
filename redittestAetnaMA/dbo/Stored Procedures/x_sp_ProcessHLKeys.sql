...
DECLARE @LoadDate				VARCHAR(50)		= '[LoadDate]'
DECLARE @SrcTblKey			VARCHAR(50)		= '[ClaimAetMAKey]'
DECLARE @ClientMbrKey		VARCHAR(50)		= '[member_id]'			
DECLARE @ClaimID				VARCHAR(50)		= '[src_clm_id]'				--s.src_clm_id
DECLARE @AndWhere				VARCHAR(50)		= 'AND revenue_cd	<> '' '' AND claim_line_id <> '' '''			-- Institutional and has no Claim Line
DECLARE @UpdateJoin			VARCHAR(100)	= 'ON	a.PatientID = b.' +  @ClientMbrKey + ' AND	a.ClaimID	= b.' +  @ClaimID + ' '	--Required
...
/***
EXEC dbo.x_sp_ProcessHLKeys '[adi].[ClaimAetMA]','2019-08-28','dbo.z_tmp_HLKeyTbl'
